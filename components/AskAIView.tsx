...
const AskAIView: React.FC<AskAIViewProps> = ({ memories }) => {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
                ...
                {messages.map((msg, i) => (
                    ...
                    >
                        <div className={`max-w-[85%] p-6 rounded-3xl border-3 ${
                            ...
                            : 'bg-[#001F3F] text-white border-white/30'
                        }`}>
                        ...
                        {msg.links && msg.links.length > 0 && (
                            <div className="flex flex-col gap-2 w-full max-w-[85%]">
                                <p className="text-xs uppercase tracking-widest text-white font-black">Related Memories:</p>
                                {msg.links.map(link => (
                                    <button 
                                        key={link.id}
                                        className="btn-outline h-16 flex items-center justify-between px-6 border-white text-white"
                                        onClick={() => {
            ...
            {/* Input Area */}
            <div className="flex flex-col gap-4 p-4 bg-[#001F3F] border-t-3 border-white/20">
                <div className="flex gap-4">
                    ...
                    <button 
                        onClick={() => handleSend()}
                        className="btn-primary w-24 bg-white text-[#001F3F]"
                        disabled={isTyping}
                    >
                ...
                <button 
                    className="btn-primary h-24 flex items-center justify-center gap-4 bg-white text-[#001F3F]"
