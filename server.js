...
async function startServer() {
  ...
  const moodleProxyHandler = async (req, res) => {
    ...
    const finalUrl = `https://online.dyellin.ac.il/webservice/rest/server.php?${params.toString()}`;
        ...
        data.forEach((section) => {
          section.modules?.forEach((mod) => {
            ...
            if (fileurl && !fileurl.includes('token=')) {
              mod.contents[0].fileurl = fileurl + (fileurl.includes('?') ? '&' : '?') + `token=${encodeURIComponent(token)}`;
            }
          });
        });
      }
